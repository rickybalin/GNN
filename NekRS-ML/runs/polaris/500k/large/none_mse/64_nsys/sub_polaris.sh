...
#    python $EXE ${ARGS}
mpiexec --envall --env TMPDIR=$PWD/tmpdir/ -n $PROCS --ppn $PROCS_PER_NODE --cpu-bind=list:24:16:8:1 nsys profile --trace=cuda,nvtx,osrt,cudnn,cublas --capture-range=cudaProfilerApi --capture-range-end=stop -o nsys_report --stats=true --show-output=true python /eagle/datascience/balin/Nek/GNN/GNN/NekRS-ML/main_mse_loss.py backend=nccl halo_swap_mode=${HALO_SWAP_MODE} gnn_outputs_path=${DATA_PATH} epochs=12
