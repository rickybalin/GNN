...
ARGS="backend=${CCL_BACKEND} halo_swap_mode=${HALO_SWAP_MODE} gnn_outputs_path=${DATA_PATH} epochs=5"
...
mpiexec --pmi=pmix --envall -n $PROCS --ppn $PROCS_PER_NODE --cpu-bind=${CPU_BIND} \
    -- /home/applenco/thapi_master/bin/iprof -l -e --trace-output $PWD/thapi-traces/$TIME_STAMP --analysis-output $PWD/thapi_report_$TIME_STAMP --trace-ranks 0,12,24,36,48  -- \
    python $EXE ${ARGS}
echo `date`
