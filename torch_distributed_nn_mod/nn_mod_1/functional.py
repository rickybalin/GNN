...
def send_recv(output_tensor_list, input_tensor_list, neighboring_procs, group=group.WORLD):
    """
    Each process scatters list of input tensors to a subset of processes in a group and
    return gathered list of tensors in output list.

    Arguments:
        out_tensor_list (list[Tensor]): list of tensors to gather one per rank. Provide empty tensor for rank not involved in communication (tensor([])). 
        input_tensor_list (list[Tensor]): List of tensors to scatter one per rank. Provide empty tensor for rank not involved in communication (tensor([])). 
        neighboring_procs (Tensor): Tensor containing communication rank IDs for the rank calling this function.
        group (ProcessGroup, optional): The process group to work on.
    ...
    """
    return _SendRecv.apply(group, neighboring_procs, output_tensor_list, *input_tensor_list)
...
class _AlltoAll(Function):
    ...
    def forward(ctx, group, out_tensor_list, *tensors):
        ...
        #print(f"[RANK {dist.get_rank()}] -- input_tensor_size_list: {ctx.input_tensor_size_list}")
        ...
        #print(f"[RANK {dist.get_rank()}] -- forward out: {tuple(out_tensor_list)}")
        return tuple(out_tensor_list)
    ...
    def backward(ctx, *grad_outputs):
        ...
        #out = (None, None) + _AlltoAll.apply(ctx.group, tensor_list, *grad_outputs)
        #out = (None,None,None,out[3])
        # if dist.get_rank() == 0:
        #     out = (None, None, None, out[3]) 
        # if dist.get_rank() == 1:
        #     out = (None, None, out[2], None)
        #print(f"[RANK {dist.get_rank()}] -- grad_outputs: {grad_outputs}")
        #print(f"[RANK {dist.get_rank()}] -- backward output: {out}")
        #return out #(None, None) + _AlltoAll.apply(ctx.group, tensor_list, *grad_outputs)
        return (None, None) + _AlltoAll.apply(ctx.group, tensor_list, *grad_outputs)

class _SendRecv(Function):
    @staticmethod
    def forward(ctx, group, neighboring_procs, out_tensor_list, *tensors):
        ctx.group = group
        ctx.neighboring_procs = neighboring_procs
        ctx.input_tensor_size_list = [
            tensors[i].size() for i in range(dist.get_world_size(group=group))
        ]
        my_rank = dist.get_rank(group=group)
        ...
        for dst in neighboring_procs:
            tmp = dist.isend(tensors[dst], dst)
            send_req.append(tmp)
        recv_req = []
        for src in neighboring_procs:
            tmp = dist.irecv(out_tensor_list[src], src)
            recv_req.append(tmp)
        for req in send_req:
            req.wait()
        for req in recv_req:
    ...
    def backward(ctx, *grad_outputs):
        ...
        return (None, None, None) + _SendRecv.apply(ctx.group, ctx.neighboring_procs, tensor_list, *grad_outputs)
