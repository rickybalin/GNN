...
ARGS="--device=cuda --iterations=20 --problem_size=large"
echo Running script $EXE
echo with arguments $ARGS
echo
echo `date`
MPICH_GPU_SUPPORT_ENABLED=0 map --profile mpiexec -n $PROCS --ppn $PROCS_PER_NODE --cpu-bind=list:24:16:8:1 python $EXE ${ARGS}
echo `date`
