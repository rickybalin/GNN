...
ARGS="--device=cuda --iterations=12 --problem_size=large"
echo Running script $EXE
echo with arguments $ARGS
echo
echo `date`
mpiexec --envall --env TMPDIR=$PWD/tmpdir/ -n $PROCS --ppn $PROCS_PER_NODE --cpu-bind=list:24:16:8:1 nsys profile --trace=cuda,nvtx,osrt,cudnn,cublas --capture-range=cudaProfilerApi --capture-range-end=stop -o nsys_report --stats=true --show-output=true python /eagle/datascience/balin/Nek/GNN/GNN/SimAI-Bench/main_nsys.py $ARGS
echo `date`
